...
// end include: preamble.js
  ...
  function _ShowShopOverlay(shopTypeStr, backendUrlStr, telegramIdStr, usernameStr) {
          ...
          // Fetch latest profile first to render purchase/equip status correctly!
          ...
          .catch(function(err) {
              console.error("Failed to load user info for shop:", err);
              // fallback render with default user data
              renderShop({ lives: 5, skins: "default", active_skin: "default", is_premium: 0 });
          });
          ...
          function renderShop(userData) {
              ...
              var ownedSkins = Array.isArray(userData.skins) ? userData.skins : (userData.skins || "default").split(",");
              var activeSkin = userData.active_skin || "default";
  
              items.forEach(function(item) {
                  var card = document.createElement("div");
                  card.className = "tg-card";
                  card.onclick = function() {
                      purchaseItem(item.id, userData);
                  };
  
                  var actionLabel = "★ " + item.price;
                  if (shopType === "skins") {
                      if (activeSkin === item.id) {
                          actionLabel = "Equipped";
                      } else if (ownedSkins.indexOf(item.id) !== -1) {
                          actionLabel = "Equip";
                      }
                  } else if (shopType === "premium" && userData.is_premium) {
                  ...
                  card.innerHTML = `
                      ...
                      <div class="tg-price" style="${actionLabel === 'Equipped' ? 'background: #2ecc71;' : (actionLabel === 'Equip' ? 'background: #3498db; color: #fff;' : '')}">
                          ${actionLabel}
                      </div>
                  `;
              ...
              function purchaseItem(itemId, currentInfo) {
                  if (shopType === "skins") {
                      var owned = Array.isArray(currentInfo.skins) ? currentInfo.skins : (currentInfo.skins || "default").split(",");
                      if (owned.indexOf(itemId) !== -1) {
                          // Already owned! Equip directly.
                          console.log("[SHOP] Skin already owned. Equipping: " + itemId);
                          equipSkinDirectly(itemId);
                          return;
                      }
                  }
  
                  // If not owned (or not a skin), proceed to Stars payment invoice
              ...
              function equipSkinDirectly(itemId) {
                  fetch(backendUrl + "/api/user/" + telegramId + "/equip-skin", {
                      method: "POST",
                      headers: {
                          "Content-Type": "application/json"
                      },
                  ...
                  .then(function(data) {
                      if (data.success) {
                          console.log("[SHOP] Skin equipped successfully: " + itemId);
                          alert("Equipped " + itemId.toUpperCase() + " aura!");
                          syncUserData();
                      } else {
                          alert("Failed to equip skin: " + (data.error || "Unknown error"));
                      }
                  })
                  .catch(function(err) {
...
var wasmImports = {
  ...
  "ShowShopOverlay": _ShowShopOverlay,
